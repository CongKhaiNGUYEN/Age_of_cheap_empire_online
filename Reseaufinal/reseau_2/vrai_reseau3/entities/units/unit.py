...
#from units.CollectionVar import *
...
#if CollectionVar.stri=="can_i":
    #fichier_py.handle_send("True")
...
class Unit:
    ...
    def attack(self, unit):
        print("ATTACK CALLED")
            ...
            #self.prop = True
        #if self.prop:
        print("owow" + str(self.tile["grid"]))
        print(unit.tile["grid"])
        if self.attacking == False:
            handle_send(f'attack,{self.tile["grid"]},{unit.tile["grid"]}')
            self.attacking=True
        ...
        if unit.hp<=0:
            ...
            self.attacked_unit = None
        #else:
            #print("property attribute is false can't attack,need to ask for permission")
            #handle_send("can_i")

    def attack_building(self, building):
        print("ATTACK BUILDING CALLED")
        if self.prop:
            self.temp_mine_tile = None
            self.temp_building = None
            self.temp_atk_unit = None
            ...
            print("cant attack building")
    
    def die(self):
        ok = self.tile['grid']
        handle_send(f'die,{ok}')
        print("DIE CALLED")
