...
from vrai_reseau2.jeu.py import *
import json


class Map:

    def __init__(self, screen, resource_manager, entities, hud, mapsize, width, height, camera, player, a=3):
        self.i = 0
        self.screen = screen
        ...
        if 0:
            self.load_save()  # mettre à 0 pour désactiver l'auto_save
        else:
        ...
        # w = Worker(self.map[4][6], self, self.resource_manager)
        # self.place_worker(w)
        # w.die()
        # w.go_close(self.map[1][3],[0])
        ...
        if self.player:
            self.ally_TH_x, self.ally_TH_y = 3, 3  # to place the GoodTH around the top of the map
            self.enemy_TH_x, self.enemy_TH_y = 46, 46
            while self.map[self.ally_TH_x][self.ally_TH_y]["water"] or self.map[self.ally_TH_x][self.ally_TH_y][
                "collision"] \
                    or self.map[self.enemy_TH_x][self.enemy_TH_y]["water"] or \
                    self.map[self.enemy_TH_x][self.enemy_TH_y]["collision"]:
            ...
            self.ally_TH_x, self.ally_TH_y = 46, 46  # to place the GoodTH around the bottom of the map
            self.enemy_TH_x, self.enemy_TH_y = 3, 3
            while self.map[self.ally_TH_x][self.ally_TH_y]["water"] or self.map[self.ally_TH_x][self.ally_TH_y][
                "collision"] \
                    or self.map[self.enemy_TH_x][self.enemy_TH_y]["water"] or \
                    self.map[self.enemy_TH_x][self.enemy_TH_y]["collision"]:
                self.ally_TH_x, self.ally_TH_y = random.randint(40, 45), random.randint(40, 45)
                self.enemy_TH_x, self.enemy_TH_y = self.mapsize - self.ally_TH_x, self.mapsize - self.ally_TH_y
        ...
        for a in self.map:
            for b in a:
                prop.append({"prop":False, "pos":b['grid']})
        print(prop)"""
    ...
    def clear_around_TH(self, area):
        ...
        self.set_all_tiles_value((self.ally_TH_x - area, self.ally_TH_y - area),
                                 (self.ally_TH_x + area, self.ally_TH_y + area), False, "")
        self.set_all_tiles_value((self.enemy_TH_x - area, self.enemy_TH_y - area),
                                 (self.enemy_TH_x + area, self.enemy_TH_y + area), False, "")
    ...
    def prop_building(func):
        def inner(self, building, grid_pos):
            print('verification started')
            building.prop = True  # initially f=False
            if building.prop:
                func(self, building, grid_pos)

            print(f"property attribute is {building.prop} for {building.name} at {building.pos}")
            handle_send(f"{building.name},{building.pos}")
            print('decoration finished')

        return inner

    def prop_worker(func):
        def inner(self, worker):
            print('verification started')
            func(self, worker)
            print(f"{worker.name},{worker.tile['grid']}")
            handle_send(f"{worker.name},{worker.tile['grid']}")
            print(f"property attribute is {worker.prop} for {worker.name}")
            print('decoration finished')

        return inner

    @prop_building
    def place_building(self, building, grid_pos):
        self.entities.append(building)
        self.buildings[grid_pos[0]][grid_pos[1]] = building
    ...
    def update(self, camera):
        ...
        if -1 < grid_pos[0] < self.mapsize and -1 < grid_pos[1] < self.mapsize:
            ...
            if self.hud.creerhorseman and self.resource_manager.is_affordable("horseman"):
                ...
                self.place_worker(ahorseman)
                # self.workers.append(ahorseman)
                self.map[self.ally_TH_x][self.ally_TH_y - 2]["troop"] = True
                time.sleep(.1)

            if self.hud.ageup and self.resource_manager.resources["wood"] > 5000 and self.resource_manager.resources[
                "stone"] > 5000 and self.resource_manager.resources["food"] > 5000 and self.resource_manager.resources[
                "gold"] > 10000:
            ...
            if self.examine_troop is not None and self.examine_troop.team == 1:  # selected worker's actions on left click
                ...
                if mouse_action[0] and self.can_place_tile(grid_pos):
                    if not (self.map[grid_pos[0]][grid_pos[1]]["collision"] or self.map[grid_pos[0]][grid_pos[1]][
                        "water"] or self.buildings[grid_pos[0]][grid_pos[1]]) and self.workers[grid_pos[0]][
                        grid_pos[1]] is None:
                    ...
                    # mining/going to a bloc
                        ...
                        if self.examine_troop.name == "worker" or self.examine_troop.name == "herobrine":
                            ...
                            self.examine_troop.go_close(self.map[grid_pos[0]][grid_pos[1]])

                    # attacking
                    elif self.workers[grid_pos[0]][grid_pos[1]] is not None and self.workers[grid_pos[0]][
                        grid_pos[1]] != self.examine_troop:
                        self.examine_troop.attacked_unit = self.workers[grid_pos[0]][grid_pos[1]]

                    # attacking building
                    elif self.buildings[grid_pos[0]][grid_pos[1]] is not None and self.buildings[grid_pos[0]][
                        grid_pos[1]].team != self.examine_troop.team:
                ...
                if self.can_place_tile(grid_pos):
                    ...
                    if mouse_action[0] and not collision and not water and not troop and not too_far:  # valid tile
                        ...
                        if self.hud.selected_tile["name"] == "Sawmill":
                            ...
                            self.place_building(ent, grid_pos)
    ...
    def draw_grass(self, map_tile):
        ...
        if tiletype == "water":
            self.grass_tiles.blit(self.tiles["water"],
    ...
    def draw(self, screen, camera):
                            ...
                            if building.name == building_name:
                                if self.allyage == 1:
                                    ...
                                    if not building.in_construction:
                                        ...
                                        back_building_position = [x_ + image.get_width() / 2 - building.maxhp / 20,
                                                                  y_ - 10, building.maxhp / 10, 5]
                                        building_position = [x_ + image.get_width() / 2 - building.maxhp / 20, y_ - 10,
                                                             building.hp / 10, 5]
                            ...
                            if worker.name == worker_name:
                                if self.allyage == 1:
                                    ...
                                    screen.blit(imageworker,
                                                ...
                                                render_pos[1] - 16 - (
                                                            worker.image.get_height() - TILE_SIZE) + camera.scroll.y))
                                    ...
                                    back_worker_position = [x_ + worker.image.get_width() / 2 - worker.maxhp / 2,
                                                            y_ - 10, worker.maxhp, 5]
                                    worker_position = [x_ + worker.image.get_width() / 2 - worker.maxhp / 2, y_ - 10,
                                                       worker.hp, 5]
                                ...
                                if self.hud.examined_troop is not None and self.hud.examined_troop.team == 1:  # examinating a troop
                                    if (x == self.examine_troop.tile["grid"][0]) and (
                                            y == self.examine_troop.tile["grid"][1]):
                                        image = self.examine_troop.image
                                        mask = pg.mask.from_surface(image).outline()
                                        mask = [(x + render_pos[
                                            0] + 32 + self.grass_tiles.get_width() / 2 + camera.scroll.x,
                                                 y + render_pos[1] - 16 - (
                                                             image.get_height() - TILE_SIZE) + camera.scroll.y)
                                                for x, y in mask]
                                        pg.draw.polygon(screen, (40, 165, 250), mask,
                                                        2)  # to hilight the selected troop
        ...
        if self.temp_tile is not None:
            ...
            iso_poly = [(x + self.grass_tiles.get_width() / 2 + camera.scroll.x, y + camera.scroll.y) for x, y in
                        iso_poly]
            if self.temp_tile["collision"] or self.temp_tile["water"] or self.temp_tile[
                "troop"]:  # can't place a building here
    ...
    def grid_to_map(self, grid_x, grid_y, r_noise):
        ...
        perlin_wood = 100 * noise.pnoise2(grid_x / (self.perlin_scale_wood), grid_y / self.perlin_scale_wood,
                                          octaves=MAP_OCTAVE, base=MAP_SEED)
        perlin_stone = 100 * noise.pnoise2(grid_x / self.perlin_scale_stone, grid_y / self.perlin_scale_stone,
                                           base=MAP_SEED)
        perlin_food = 100 * noise.pnoise2(grid_x / self.perlin_scale_food, grid_y / self.perlin_scale_food,
                                          base=MAP_SEED)
        perlin_gold = 100 * noise.pnoise2(grid_x / self.perlin_scale_gold, grid_y / self.perlin_scale_gold,
                                          base=MAP_SEED)
        perlin_water = 100 * noise.pnoise2(grid_x / self.perlin_scale_water, grid_y / self.perlin_scale_water,
                                           octaves=MAP_OCTAVE, base=MAP_SEED)
        ...
        out = {
            ...
            "resources": 1000 if (tile == "wood" or tile == "food") else (
                5000 if tile == "gold" or tile == "stone" else 0),
            ...
            for grid_x in range(self.mapsize):
                for grid_y in range(self.mapsize):
                    ...
                    if tiletype == 'water':
                        ...
                        self.grass_tiles.blit(self.tiles["sand"],
                                              (render_pos[0] + self.grass_tiles.get_width() / 2, render_pos[1]))
